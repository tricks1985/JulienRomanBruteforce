...
with open('2bis_noContracts.txt') as file:
    addrs = file.readlines()


# get addrs that have 3 Txs or less (ie nonce <= 3)
for addr in addrs:
    addr = addr.replace("\n","")

    if web3_eth.eth.get_transaction_count(addr) <= 3:
        with open("2ter_noNonce.txt", "a") as file:
            file.write(addr+"\n")
