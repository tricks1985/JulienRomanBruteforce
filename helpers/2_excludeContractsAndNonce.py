...
# get addrs that didn't sign any Txs (ie nonce = 0)
# will exclude contracts too because contracts have nonce = 1
for addr in addrs:
    addr = Web3.toChecksumAddress(addr.replace("\n",""))

    if not web3_eth.eth.get_transaction_count(addr) :
        with open("2_noContractsAndNonce.txt", "a") as file:
            file.write(addr+"\n")
